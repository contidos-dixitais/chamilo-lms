...
class SmowlTokenRequest
{
    ...
    public function validateClientAssertion($clientAssertion)
    {
        ...
        $this->tool = Database::getManager()
            ->getRepository('ChamiloPluginBundle:Smowl\SmowlTool')
            ->findOneBy(['clientId' => $claims['sub']]);

        if (!$this->tool ||
            $this->tool->getVersion() !== Smowl::V_1P3 ||
    ...
    public function validateScope($scope)
    {
        ...
        $services = Smowl::getAdvantageServices($this->tool);

        $requested = explode(' ', $scope);
        $allowed = [];

        /** @var SmowlAdvantageService $service */
