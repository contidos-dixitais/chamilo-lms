...
/**
 * Class SmowlAssignmentGradesService.
 */
class SmowlAssignmentGradesService extends SmowlAdvantageService
{
    const AGS_NONE = 'none';
     ...
     *
     * @return SmowlAdvantageServiceResource
     */
    public static function getResource(Request $request, JsonResponse $response)
    {
        ...
        if (count($parts) === 2 && 'lineitems' === $parts[2]) {
            $resource = new SmowlLineItemsResource(
                $request->query->get('t'),
                $parts[1]
            );
        }
        ...
        if (count($parts) === 3 && 'lineitems' === $parts[2]) {
            $resource = new SmowlLineItemResource(
                $request->query->get('t'),
                $parts[1],
                $parts[3]
            );
        ...
        if (isset($parts[4]) && 'results' === $parts[4]) {
            $resource = new SmowlResultsResource(
                $request->query->get('t'),
                $parts[1],
                $parts[3]
            );
        ...
        if (isset($parts[4]) && 'scores' === $parts[4]) {
            $resource = new SmowlScoresResource(
                $request->query->get('t'),
                $parts[1],
                $parts[3]
            );
    ...
    {
        $base = api_get_path(WEB_PLUGIN_PATH).'smowl/ags2.php';
        $resource = str_replace(
            'context_id',
            $contextId,
            SmowlLineItemsResource::URL_TEMPLATE
        );
    ...
    public static function getLineItemUrl($contextId, $lineItemId, $toolId)
    {
        ...
        $resource = str_replace(
            ...
            SmowlLineItemResource::URL_TEMPLATE
        );
