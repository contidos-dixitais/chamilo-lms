...
use Chamilo\PluginBundle\Entity\Smowl\LineItem;
...
/**
 * Class SmowlResultsResource.
 */
class SmowlResultsResource extends SmowlAdvantageServiceResource
{
    ...
    /**
     * SmowlResultsResource constructor.
     *
     * @param int $toolId
     * @param int $courseId
    ...
    public function __construct($toolId, $courseId, $lineItemId)
    {
        ...
        $this->lineItem = Database::getManager()->find('ChamiloPluginBundle:Smowl\LineItem', (int)$lineItemId);
    }
    ...
    public function validate()
    {
        ...
        if ($this->request->server->get('HTTP_ACCEPT') !== SmowlAssignmentGradesService::TYPE_RESULT_CONTAINER) {
    ...
    public function process()
    {
        switch ($this->request->getMethod()) {
            case Request::METHOD_GET:
                $this->validateToken(
                    [SmowlAssignmentGradesService::SCOPE_RESULT_READ]
                );
                $this->processGet();
                break;
            default:
    ...
    private function processGet()
    {
        ...
        $this->response->headers->set('Content-Type', SmowlAssignmentGradesService::TYPE_RESULT_CONTAINER);
    ...
    private function getGetData(array $results)
    {
        ...
        foreach ($results as $result) {
            $lineItemEndPoint = SmowlAssignmentGradesService::getLineItemUrl(
                $this->course->getId(),
                $this->lineItem->getId(),
                $this->tool->getId()
            );
    ...
    private function setLinkHeaderToGet($userId, $limit, $page = 0)
    {
        ...
        foreach ($links as $rel => $linkPage) {
            $url = SmowlAssignmentGradesService::getResultsUrl(
                $this->course->getId(),
                $this->lineItem->getId(),
                $this->tool->getId(),
