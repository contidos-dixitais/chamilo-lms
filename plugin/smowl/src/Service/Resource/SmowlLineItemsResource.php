...
/**
 * Class SmowlLineItemsResource.
 */
class SmowlLineItemsResource extends SmowlAdvantageServiceResource
{
    ...
    public function validate()
    {
        ...
        $isMediaTypeAllowed = in_array(
            ...
            [
                SmowlAssignmentGradesService::TYPE_LINE_ITEM_CONTAINER,
                SmowlAssignmentGradesService::TYPE_LINE_ITEM,
            ]
        );
    ...
    private function processPost()
    {
        ...
        $data['id'] = SmowlAssignmentGradesService::getLineItemUrl(
            $this->course->getId(),
            $lineItem->getId(),
            $this->tool->getId()
        );

        $this->response->headers->set('Content-Type', SmowlAssignmentGradesService::TYPE_LINE_ITEM);
    ...
    private function setLinkHeaderToGet(ArrayCollection $lineItems, $resourceLinkId, $resourceId, $tag, $limit, $page)
    {
        ...
        foreach ($links as $rel => $linkPage) {
            $url = SmowlAssignmentGradesService::getLineItemsUrl(
                $this->course->getId(),
                $this->tool->getId(),
                [
    ...
    private function getGetData(ArrayCollection $lineItems)
    {
        ...
        foreach ($lineItems as $lineItem) {
            ...
            $item['id'] = SmowlAssignmentGradesService::getLineItemUrl(
                $this->course->getId(),
                $lineItem->getId(),
                $this->tool->getId()
            );
