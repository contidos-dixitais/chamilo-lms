...
/**
 * Class SmowlContextMembershipResource.
 */
class SmowlContextMembershipResource extends SmowlAdvantageServiceResource
{
    ...
    /**
     * SmowlContextMembershipResorce constructor.
     *
     * @param int $toolId
     * @param int $courseId
     * @param int $sessionId
    ...
    {
        if ($this->request->server->get('HTTP_ACCEPT') !== SmowlNamesRoleProvisioningService::TYPE_MEMBERSHIP_CONTAINER) {
    ...
    public function process()
    {
        switch ($this->request->getMethod()) {
            case Request::METHOD_GET:
                $this->validateToken(
                    [SmowlNamesRoleProvisioningService::SCOPE_CONTEXT_MEMBERSHIP_READ]
                );
                $this->processGet();
                break;
            default:
    ...
    private function processGet()
    {
        ...
        $this->response->headers->set('Content-Type', SmowlNamesRoleProvisioningService::TYPE_MEMBERSHIP_CONTAINER);
    ...
    private function getGetData(array $members)
    {
        ...
        foreach ($members as $member) {
            ...
            $dataMember = [
                'status' => $user->isActive()
                    ? SmowlNamesRoleProvisioningService::USER_STATUS_ACTIVE
                    : SmowlNamesRoleProvisioningService::USER_STATUS_INACTIVE,
                'user_id' => SmowlPlugin::getLaunchUserIdClaim($this->tool, $user),
                'lis_person_sourcedid' => Smowl::getPersonSourcedId($platformDomain, $user),
                'lti11_legacy_user_id' => SmowlPlugin::generateToolUserId($user->getId()),
            ];
        ...
        return [
            'id' => api_get_path(WEB_PLUGIN_PATH)
                ."smowl/nrps2.php/{$this->course->getId()}/memberships?"
                .http_build_query(
                    [
                        't' => $this->tool->getId(),
    ...
    private function setLinkHeaderToGet($status, $limit, $page = 0)
    {
        ...
        if ($page + 1 < ceil($count / $limit)) {
            $url = SmowlNamesRoleProvisioningService::getUrl(
                $this->tool->getId(),
                $this->course->getId(),
