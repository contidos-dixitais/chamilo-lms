...
try {
    ...
    if ($scope !== 'openid') {
        throw SmowlAuthException::invalidScope();
    }

    if ($responseType !== 'id_token') {
        throw SmowlAuthException::unsupportedResponseType();
    }

    if (empty($responseMode)) {
        throw SmowlAuthException::missingResponseMode();
    }

    if ($responseMode !== 'form_post') {
        throw SmowlAuthException::invalidRespondeMode();
    }
    ...
    if ($tool->getClientId() != $clientId) {
        throw SmowlAuthException::unauthorizedClient();
    }

    $user = api_get_user_entity(api_get_user_id());

    if (SmowlPlugin::getLaunchUserIdClaim($tool, $user) != $loginHint) {
        throw SmowlAuthException::accessDenied();
    }

    if ($redirectUri !== $tool->getRedirectUrl()) {
        throw SmowlAuthException::unregisteredRedirectUri();
    }
    ...
    $jwtContent = [];
    $jwtContent['iss'] = SmowlPlugin::getIssuerUrl();
    $jwtContent['sub'] = SmowlPlugin::getLaunchUserIdClaim($tool, $user);
    ...
    if (DRH === $user->getStatus()) {
        $roleScopeMentor = SmowlPlugin::getRoleScopeMentor($user, $tool);
    ...
    if ($tool->isActiveDeepLinking()) {
        $jwtContent['https://purl.imsglobal.org/spec/lti/claim/message_type'] = 'SmowlDeepLinkingRequest';
        $jwtContent['https://purl.imsglobal.org/spec/lti-dl/claim/deep_linking_settings'] = [
            ...
            'deep_link_return_url' => $webPluginPath.'smowl/item_return2.php',
        ];

    } else {
        $jwtContent['https://purl.imsglobal.org/spec/lti/claim/message_type'] = 'SmowlResourceLinkRequest';
        ...
        $jwtContent['https://purl.imsglobal.org/spec/lti/claim/lis'] = [
            'person_sourcedid' => Smowl::getPersonSourcedId($platformDomain, $user),
            'course_section_sourcedid' => Smowl::getCourseSectionSourcedId($platformDomain, $course, $session),
        ];
        ...
        if (!empty($advServices)) {
            if (SmowlAssignmentGradesService::AGS_NONE !== $advServices['ags']) {
                $agsClaim = [
                    'scope' => [
                        SmowlAssignmentGradesService::SCOPE_LINE_ITEM,
                        SmowlAssignmentGradesService::SCOPE_LINE_ITEM_READ,
                    ],
                    'lineitems' => SmowlAssignmentGradesService::getLineItemsUrl(
                        $course->getId(),
                        $tool->getId()
                    ),
                ];
                ...
                if ($tool->getLineItems()->count() === 1) {
                    $agsClaim['lineitem'] = SmowlAssignmentGradesService::getLineItemUrl(
                        $course->getId(),
            ...
            if (SmowlNamesRoleProvisioningService::NRPS_NONE !== $advServices['nrps'] &&
                api_is_allowed_to_edit(false, false, true)
            ) {
                $nrpsClaim = [
                    'context_memberships_url' => SmowlNamesRoleProvisioningService::getUrl(
                        $tool->getId(),
                        $course->getId(),
    ...
    $jwt = JWT::encode(
        $jwtContent,
        $platform->getLicenseKey(),
        'RS256',
        $platform->getEntityName()
    );
...
} catch (SmowlAuthException $authException) {
    $params = [
